...
download() {
    ...
    if [ "$DL" = "-d" ]; then
        axel -a -k -n 16 --header=Referer:"$1" "$2" -o "$url.mp4"
        return
    fi
    ch=$(printf "play\ndownload" | fzf)
    [ -z "$ch" ] && exit 0
...
for ((i = 1; i <= episode_num; i++)); do
    ...
    url=$(echo "$urlo" | sed -E 's/([0-9]+)$//')$i

    # Get the embed URL for the current episode
    refrdl=$(curl -A "$agent" -s "$base_url/videos/$url" | sed -nE 's/.*iframe src="(.*)" al.*/\1/p')

    # Display a message while fetching embed links
    printf "\33[2K\r\033[1;34mFetching Embed links for episode $i"

    # Get the list of embed links for the current episode
    respdl="$(curl -A "$agent" -s "https:$refrdl" | sed -nE 's/.*class="container-(.*)">/\1/p ; s/.*class="wrapper container-(.*)">/\1/p ; s/.*class=".*videocontent-(.*)">/\1/p ; s/.*data-value="(.*)">.*/\1/p ; s/.*data-status="1".*data-video="(.*)">.*/\1/p')"

    # Get the fembed link for the current episode
    fb_link=$(printf "$respdl" | sed -n "s_.*fembed.*/v/__p")
